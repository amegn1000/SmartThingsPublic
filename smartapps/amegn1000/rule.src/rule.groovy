...
definition(
	name: "Rule",
	namespace: "amegn1000",
	author: "Arnaud Gasquy",
	description: "Rule",
	category: "Convenience",
	parent: "amegn1000:Rule Machine",
	iconUrl: "https://s3.amazonaws.com/smartapp-icons/Partner/App-SmartBlock.png",
	iconX2Url: "https://s3.amazonaws.com/smartapp-icons/Partner/App-SmartBlock.png",
)
...
def mainPage()
{
	try
		...
		if (state.isExpert)
        	{
            state.cstCmds = parent.getCommands()
            }
		else
        	{
            state.cstCmds = []
	...
	if (state.howManyT > 1 || state.isTrig)
    	{
        myTitle = "Define a Trigger"
        }
	else if (state.howMany > 1)
    	{
        myTitle = "Define a Rule"
        }
	else if (app.label != null)
    	{
        myTitle = "Define Actions"
        }
	...
	dynamicPage(name: "mainPage", title: myTitle, uninstall: myUninstall, install: myUninstall) {
		if (state.isTrig)
			...
			section (title: "Restrictions", hidden: hideOptionsSection(), hideable: true)
				...
				input (name: "days", type: "enum", title: "Only on certain days of the week:", description: "select days of the week", defaultValue: null, multiple: true, required: false, submitOnChange: false,
					options: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"])
				input (name: "modesY", type: "mode", title: "Only when mode is:", description: "select mode(s)", defaultValue: null, multiple: true, required: false, submitOnChange: false)
				input (name: "disabled", type: "capability.switch", title: "Switch to disable trigger when ON:", description: "select switch", defaultValue: null, multiple: false, required: false, submitOnChange: false)
				}    
			}
        else if (state.isRule)
        	{
			section()
            	{
			...
			section(title: "Restrictions", hidden: hideOptionsSection(), hideable: true)
            	{
				input (name: "modesZ", type: "mode", title: "Evaluate only when mode is:", description: "select mode(s)", defaultValue: null, multiple: true, required: false, submitOnChange: false)
				input (name: "disabled", type: "capability.switch", title: "Switch to disable rule when ON:", description: "select switch", defaultValue: null, multiple: false, required: false, submitOnChange: false)
				}   
			} 
        else if (state.howMany > 1 && state.howManyT in [null, 1])
        	{
            getRule()
            }
        else if (state.howManyT > 1 && state.howMany in [null, 1])
        	{
            getTrigger()
            }
		else if (state.howManyT > 1)
        	{
        	getCTrigger()
            }
...
def getMoreOptions()
{
	section(title: "Restrictions", hidden: hideOptionsSection(), hideable: true)
        ...
        if (disabled)
        	{
        	input (name: "disabledOff", type: "bool", title: "Disable when Off?\n(On is default)", description: "select answer", defaultValue: false, multiple: false, required: false, submitOnChange: false)
        	}
        def privy = state.private
        input (name: "usePrivateDisable", type: "bool", title: "Enable/Disable with private Boolean? [$privy]", description: "select answer", defaultValue: false, multiple: false, required: false, submitOnChange: false)
		}    
}
...
def certainTime()
{
	dynamicPage (name: "certainTime", title: "Between two times", uninstall: false)
		...
		section()
        	{
			input (name: "startingX", type: "enum", title: "Starting at:", description: "select answer", defaultValue: "A specific time", multiple: false, required: false, submitOnChange: true,
            	options: ["A specific time", "Sunrise", "Sunset"])
			if (startingX in [null, "A specific time"])
            	{
                input (name: "starting", type: "time", title: "Start time:", description: "enter time", range: "*..*", defaultValue: null, multiple: false, required: false, submitOnChange: false)
                }
			else
            	{
				if (startingX == "Sunrise")
                	{
                    input (name: "startSunriseOffset", type: "number", title: "Sunrise Offset in minute(s):\n(+/-)", description: "enter minute(s)", range: "*..*", defaultValue: null, multiple: false, required: false, submitOnChange: false)
					}
                else if (startingX == "Sunset")
                	{
                    input (name: "startSunsetOffset", type: "number", title: "Sunset Offset in minute(s):\n(+/-)", description: "enter minute(s)", range: "*..*", defaultValue: null, multiple: false, required: false, submitOnChange: false)
					}
                }
			}
		section()
        	{
			input (name: "endingX", type: "enum", title: "Ending at:", description: "select answer", defaultValue: "A specific time", multiple: false, required: false, submitOnChange: true,
            	 options: ["A specific time", "Sunrise", "Sunset"])
			if (endingX in [null, "A specific time"])
            	{
                input (name: "ending", type: "time", title: "End time:", description: "enter time", range: "*..*", defaultValue: null, multiple: false, required: false, submitOnChange: false)
				}
			else
            	{
				if (endingX == "Sunrise")
                	{
                    input (name: "endSunriseOffset", type: "number", title: "Sunrise Offset in minute(s):\n(+/-)", description: "enter minute(s)", range: "*..*", defaultValue: null, multiple: false, required: false, submitOnChange: false)
					}
                else if (endingX == "Sunset")
                	{
                    input (name: "endSunsetOffset", type: "number", title: "Sunset Offset in minute(s):\n(+/-)", description: "enter minute(s)", range: "*..*", defaultValue: null, multiple: false, required: false, submitOnChange: false)
					}
                }
			}
        }
...
{
	def myOptions = null
	if (state.isRule || !isTrig)
    	{
        myOptions = ["Acceleration", "Battery", "Carbon monoxide detector", "Contact", "Days of week", "Dimmer level", "Energy meter", "Garage door", "Humidity", "Illuminance", "Lock", "Mode", "Motion", "Power meter", "Presence", "Rule truth", "Smart Home Monitor", "Smoke detector", "Switch", "Temperature", "Private Boolean", "Door", "Thermostat Mode", "Thermostat State", "Time of day", "Water sensor", "Music player"]
		}
    if (state.isTrig || isTrig)
    	{
        myOptions = ["Acceleration", "Battery", "Button", "Carbon monoxide detector", "Certain Time", "Periodic", "Contact", "Dimmer level", "Energy meter", "Garage door", "Humidity", "Illuminance", "Lock", "Mode", "Motion", "Physical Switch", "Power meter", "Presence", "Routine", "Rule truth", "Smart Home Monitor", "Smoke detector", "Switch", "Temperature", "Door", "Thermostat Mode", "Thermostat State", "Water sensor", "Private Boolean", "Music player"]
		}
...
def getDevs(myCapab, dev, multi)
{
    ...
    def thisCapab = ""
    def thisImage = ""
	def thisValue = settings.find {it.key == dev}
	switch (myCapab)
    	{
		case "Switch":
			thisName = "Switch" + (thisValue ? (thisValue.value.size() > 1 ? "es" : "") : "") //(multi ? "es" : "")
			thisCapab = "switch"
            thisImage = "st.Home.home30"
			break
		case "Physical Switch":
			thisName = "Switch" + (multi ? "es" : "")
			thisCapab = "switch"
            thisImage = "st.switches.switch.on"
			break
		case "Motion":
			thisName = "Motion Sensor"  + (multi ? "s" : "")
			thisCapab = "motionSensor"
            thisImage = "st.motion.motion.active"
			break
		case "Acceleration":
			thisName = "Acceleration Sensor"  + (multi ? "s" : "")
			thisCapab = "accelerationSensor"
            thisImage = "st.motion.acceleration.active"
			break        
		case "Contact":
			thisName = "Contact Sensor" + (multi ? "s" : "")
			thisCapab = "contactSensor"
            thisImage = "st.contact.contact.closed"
			break
		case "Presence":
			thisName = "Presence Sensor" + (multi ? "s" : "")
			thisCapab = "presenceSensor"
            thisImage = "st.presence.tile.presence-default"
			break
		case "Garage door":
			thisName = "Garage Door"  + (multi ? "s" : "")
			thisCapab = "garageDoorControl"
            thisImage = "st.doors.garage.garage-closed"
			break
		case "Door":
			thisName = "Door" + (multi ? "s" : "")
			thisCapab = "doorControl"
            thisImage = "st.categories.doorsAndLocks"
			break
		case "Lock":
			thisName = "Lock" + (multi ? "s" : "")
			thisCapab = "lock"
            thisImage = "st.locks.lock.locked"
			break
		case "Dimmer level":
			thisName = "Dimmer" + (multi ? "s" : "")
			thisCapab = "switchLevel"
            thisImage = "st.Lighting.light11"
			break
		case "Temperature":
			thisName = "Temperature Sensor" + (multi ? "s" : "")
			thisCapab = "temperatureMeasurement"
            thisImage = "st.alarm.temperature.normal"
			break
		case "Thermostat Mode":
			thisName = "Thermostat" + (multi ? "s" : "")
			thisCapab = "thermostat"
            thisImage = "st.thermostat.ac.air-conditioning"
			break
		case "Thermostat State":
			thisName = "Thermostat" + (multi ? "s" : "")
			thisCapab = "thermostat"
            thisImage = "st.thermostat.auto"
			break
		case "Humidity":
			thisName = "Humidity Sensor" + (multi ? "s" : "")
			thisCapab = "relativeHumidityMeasurement"
            thisImage = "st.Weather.weather12"
			break
		case "Illuminance":
			thisName = "Illuminance Sensor" + (multi ? "s" : "")
			thisCapab = "illuminanceMeasurement"
            thisImage = "st.illuminance.illuminance.bright"
			break
		case "Energy meter":
			thisName = "Energy Meter" + (multi ? "s" : "")
			thisCapab = "energyMeter"
            thisImage = "st.lights.philips.hue-multi"
			break
		case "Power meter":
			thisName = "Power Meter" + (multi ? "s" : "")
			thisCapab = "powerMeter"
            thisImage = "st.lights.multi-light-bulb-on"
			break
		case "Carbon monoxide detector":
			thisName = "CO Detector" + (multi ? "s" : "")
			thisCapab = "carbonMonoxideDetector"
            thisImage = "st.alarm.carbon-monoxide.carbon-monoxide"
			break
		case "Smoke detector":
			thisName = "Smoke Detector" + (multi ? "s" : "")
			thisCapab = "smokeDetector"
            thisImage = "st.alarm.smoke.smoke"
			break
		case "Water sensor":
			thisName = "Water Sensor" + (multi ? "s" : "")
			thisCapab = "waterSensor"
            thisImage = "st.alarm.water.wet"
			break
		case "Music player":
			thisName = "Music Player"  + (multi ? "s" : "")
			thisCapab = "musicPlayer"
            thisImage = "st.Electronics.electronics12"
			break
		case "Battery":
			thisName = multi ? "Batteries" : "Battery"
			thisCapab = "battery"
            thisImage = "st.quirky.spotter.quirky-spotter-plugged"
            break
		case "Rule truth":
			def theseRules = parent.ruleList(app.label)
			def result = input dev, "enum", image: "st.unknown.thing.thing-circle", title: "Rules", required: true, multiple: multi, submitOnChange: true, options: theseRules.sort()
			return result
		}
	def result = input dev, "capability.$thisCapab", image: thisImage, title: thisName, required: true, multiple: multi, submitOnChange: true
}
...
{
	def result = input "Rel$myDev", "enum", title: "Choose Comparison", required: true,
    	options: ["=": "Equal (=)", "!=": "Diffrent (!=)", "<": "Inferior (<)", ">": "Superior (>)", "<=": "Inferior or Equal (<=)", "<=": "Superior or Equal (>=)"]
}

def getButton(dev)
{
	def numNames = ["one": "One (1)", "two": "Two (2)", "three": "Three (3)", "four": "Four (4)", "five": "Five (5)", "six": "Six (6)", "seven": "Seven (7)", "eight": "Eight (8)", "nine": "Nine (9)", "ten": "Ten (10)",
    "eleven": "Eleven (11)", "twelve": "Twelve (12)", "thirteen": "Thirteen (13)", "fourteen": "Fourteen (14)", "fifteen": "Fifteen (15)", "sixteen": "Sixteen (16)", "seventeen": "Seventeen (17)", "eighteen": "Eighteen (18)", "nineteen": "Nineteen (19)", "twenty": "Twenty (20)"]
 	def result = input "$dev", "capability.button", title: "Button Device:", description: "select button", defaultValue: null, multiple: false, required: false, submitOnChange: true
	def thisDev = settings.find{it.key == "$dev"}
	if (thisDev)
    	{
		input "numButtons$dev", "number", title: "Number of buttons? (Default 4)", description: "enter number of button(s)\n1 to 20", range: "1..20", defaultValue: 4, multiple: false, required: false, submitOnChange: true
...
def getState(myCapab, n, isTrig)
{
    ...
    def statePhrase = isTrig ? "triggers" : "state is"
    def stateNPhrase = isTrig ? "becomes" : "is"
	def isRule = state.isRule || (state.howMany > 1 && !isTrig)
	def days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
    def contactOptions = isTrig ? ["open": "Open", "closed": "Close"] : ["open": "Opened", "closed": "Closed"] 
    def presenceOptions = isTrig ? ["present": "Arrive", "not present": "Leave"] : ["present": "Present", "not present": "Not Present"]
    def doorOptions = isTrig ? ["open": "Open", "closed": "Close", "opening": "Opening", "closing": "Closing", "unknown": "Unknown"] : ["open": "Opened", "closed": "Closed", "opening": "Opening", "closing": "Closing", "unknown": "Unknown"]
    def musicPlayerOptions = isTrig ? ["playing": "Play", "paused": "Pause","stopped": "Stop"] : ["playing": "Playing", "paused": "Paused","stopped": "Stopped"]
	def SmartHomeMonitorOptions = isTrig ? ["away" : "Arm (Away)", "stay" : "Arm (Stay)", "off" : "Disarm"] : ["away" : "Armed (Away)", "stay" : "Armed (Stay)", "off" : "Disarmed"]
	//def whatState = settings.find {it.key == myState}
    if (myCapab == "Switch")
    	{
		result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "on", multiple: false, required: false, submitOnChange: true,
			options: ["on": "ON", "off": "OFF"]
        }
	else if (myCapab == "Physical Switch")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "on", multiple: false, required: false,
			options: ["on": "ON", "off": "OFF"]
		}
    else if (myCapab == "Motion")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "active", multiple: false, required: false,
	        options: ["active": "Active", "inactive": "Inactive"]
		}
    else if (myCapab == "Acceleration")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "active", multiple: false, required: false,
	        options: ["active": "Active", "inactive": "Inactive"]
		}
    else if (myCapab == "Contact")
    	{
    	result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "open", multiple: false, required: false,
        	options: contactOptions
		}
    else if (myCapab == "Presence")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "present", multiple: false, required: false,
        	options: presenceOptions
		}
    else if (myCapab == "Garage door")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "open", multiple: false, required: false,
        	options: doorOptions
		}
    else if (myCapab == "Door")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "open", multiple: false, required: false,
        	options: doorOptions
		}
    else if (myCapab == "Lock")
    	{
    	result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "unlocked", multiple: false, required: false,
			options: [ "unlocked": "Unlocked", "locked": "Locked"]
		}
    else if (myCapab == "Thermostat Mode")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:",description: "select state", defaultValue: "auto", multiple: false, required: false,
        	options: ["heat": "Heat", "cool": "Cool", "auto": "Auto", "off": "Off", "emergency heat": "emergency Heat"]
		}
    else if (myCapab == "Thermostat State")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:",description: "select state", defaultValue: "idle", multiple: false, required: false,
        	options: ["idle": "Idle", "cooling": "Cooling", "pending cool": "Pending Cool",  "heating": "Heating", "pending heat": "Pending Heat", "vent economizer": "Vent Economizer", "fan only": "Fan Only"]
		}
    else if (myCapab == "Carbon monoxide detector")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "detected", multiple: false, required: false,
        	options: ["clear": "Clear", "detected": "Detected", "tested": "Tested"]
		}
	else if (myCapab == "Smoke detector")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "detected", multiple: false, required: false,
        	options: ["clear": "Clear", "detected": "Detected", "tested": "Tested"]
		}
    else if (myCapab == "Water sensor")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "wet", multiple: false, required: false,
        	options: ["dry": "Dry", "wet": "Wet"]
		}
    else if (myCapab == "Button")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "pushed", multiple: false, required: false,
        	options: ["pushed": "Pushed", "held": "held"]
		}
    else if (myCapab == "Rule truth")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "true", multiple: false, required: false,
        	options: ["true": "True", "false": "False"]
		}
    else if (myCapab == "Music player")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "playing", multiple: false, required: false,
        	options: musicPlayerOptions
		}
    else if (myCapab == "Private Boolean")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "true", multiple: false, required: false,
			options: ["true": "True", "false": "False"]
		}
    else if (myCapab == "Smart Home Monitor")
    	{
        result = input myState, "enum", title: "$myCapab $statePhrase:", description: "select state", defaultValue: "true", multiple: false, required: false,
        	options: SmartHomeMonitorOptions
		}
		...
		if (myDev && myDev.value)
			...
			if (myCapab == "Temperature")
            	{
                result = input myState, "decimal", title: "$myCapab offset:", range: "*..*", description: "enter temperature offset (-X.X°F to X.X°F)", defaultValue: 0, multiple: false, required: false
				}
            else if (myCapab == "Humidity")
            	{
                result = input myState, "number", title: "$myCapab offset:", range: "-100..100", description: "enter humidity offset (-100% to 100%)", defaultValue: 0, multiple: false, required: false
				}
            else if (myCapab == "Illuminance")
            	{
                result = input myState, "number", title: "$myCapab offset:", range: "*..*", description: "enter illuminance offset (-X/lux to X/lux)", defaultValue: 0, multiple: false, required: false
				}
            else if (myCapab == "Dimmer level")
            	{
                result = input myState, "number",title: "$myCapab offset:", range: "-100..100", description: "enter dim level offset (-100% to 100%)", defaultValue: 0, multiple: false, required: false
				}
            else if (myCapab == "Energy meter")
            	{
                result = input myState, "decimal", title: "$myCapab offset:", range: "*..*", description: "enter energy offset (-X.X/kwh to X.X/kwh)", defaultValue: 0, multiple: false, required: false
				}
            else if (myCapab == "Power meter")
            	{
                result = input myState, "decimal", title: "$myCapab offset:", range: "*..*", description: "enter power offset (-X.X/kwh to X.X/kwh)", defaultValue: 0, multiple: false, required: false
				}
            else if (myCapab == "Battery")
            	{
                result = input myState, "number", title: "$myCapab offset:", range: "-100..100", description: "enter battery offset (-100% to 100%)", defaultValue: 0, multiple: false, required: false
				}
        	}
		else if (myCapab == "Temperature")
        	{
            result = input myState, "decimal", title: "$myCapab $stateNPhrase:", range: "*..*", description: "enter battery offset (-X.X°F to X.X°F)", defaultValue: 0, multiple: false, required: false
			}
...
def certainTimeX()
{
	dynamicPage(name: "certainTimeX", title: "Between two times", uninstall: false)
		...
		section()
			...
			if (startingXX in [null, "A specific time"])
            	{
                input "startingA", "time", title: "Start time", required: false
                }
			else
            	{
				if (startingXX == "Sunrise")
                	{
                    input "startSunriseOffsetX", "number", range: "*..*", title: "Offset in minutes (+/-)", required: false
					}
                else if (startingXX == "Sunset")
                	{
                    input "startSunsetOffsetX", "number", range: "*..*", title: "Offset in minutes (+/-)", required: false
				}
            }
		}
		section() {
